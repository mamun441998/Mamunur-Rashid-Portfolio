...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-[#0a0a0a] border border-white/10 hover:border-[#00ffc2]/50 rounded-2xl overflow-hidden flex flex-col justify-between group transition-all duration-300 relative shadow-lg hover:shadow-[0_0_25px_rgba(0,255,194,0.15)]"
    >
      ...
      <div className="p-6 flex flex-col flex-grow justify-between z-10">
        <div>
          <h3 className="text-xl font-bold text-white font-space-grotesk group-hover:text-[#00ffc2] transition-colors line-clamp-1">
            {project.title}
          </h3>

          <p className="text-gray-400 text-sm mt-3 leading-relaxed font-inter line-clamp-3">
            {project.description}
          </p>
        </div>
        ...
        <div className="mt-6 pt-4 border-t border-white/5 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {techList.map((tech) => (
              <span
                key={tech}
                className="px-2.5 py-1 text-[11px] font-mono rounded-md bg-[#161616] text-[#00ffc2] border border-[#00ffc2]/20"
              >
                {tech}
              </span>
            ))}
          ...
          <div className="flex items-center gap-4 pt-2">
            {project.github_url && (
              <a
                ...
                className="flex items-center gap-2 text-xs font-mono text-gray-300 hover:text-[#00ffc2] transition-colors"
              >
            ...
            {project.project_url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 text-xs font-mono text-[#00ffc2] hover:underline transition-all ml-auto"
              >
                Live Preview
