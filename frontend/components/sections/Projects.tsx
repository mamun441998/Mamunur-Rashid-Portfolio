...
export default function Projects() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/3 right-10 w-[450px] h-[450px] bg-[var(--color-accent)]/5 blur-[140px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl w-full z-10 flex flex-col items-center">
        ...
        <div className="flex flex-col items-center mb-12 text-center">
          ...
          >
            Featured <span className="text-[var(--color-accent)] text-glow">Projects</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            ...
            {(['All', 'SaaS', 'Web Apps'] as const).map((tab) => (
              <button
                ...
                className={`px-5 py-2 text-xs md:text-sm font-medium rounded-full transition-all duration-300 border cursor-pointer ${
                  filter === tab
                    ? 'bg-[var(--color-accent)]/10 border-[var(--color-accent)] text-[var(--color-accent)] shadow-[0_0_15px_rgba(0,255,194,0.2)]'
                    : 'bg-[var(--input)] border-[var(--bd)] text-[var(--txt-2)] hover:border-[var(--color-border-hover)] hover:text-[var(--txt)]'
                }`}
              >
                {tab}
              </button>
        ...
        {!isLoading && !isError && filteredProjects.length === 0 && (
          <div className="text-center py-12 text-[var(--txt-2)] font-mono text-sm">
        ...
        >
          <Link
            ...
            className="group relative inline-flex items-center gap-3 px-8 py-4 rounded-2xl overflow-hidden font-mono text-sm font-semibold tracking-wide border border-[var(--color-accent)]/40 bg-white/[0.03] text-[var(--txt)] shadow-[0_0_20px_rgba(0,255,194,0.15)] transition-all duration-500 hover:border-[var(--color-accent)] hover:shadow-[0_0_35px_rgba(0,255,194,0.35)] active:scale-95"
          >
            {/* Glass Liquid Water Fill Wave Effect */}
            <span className="absolute inset-0 w-full h-full bg-[var(--color-accent)] translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out z-0 opacity-90 rounded-2xl" />

            {/* Glowing Particle Backdrop */}
            <span className="absolute -top-10 -left-10 w-24 h-24 bg-[var(--color-accent)]/30 rounded-full blur-xl group-hover:opacity-0 transition-opacity" />

            {/* Button Content */}
            <span className="relative z-10 flex items-center gap-2.5 text-[var(--txt)] group-hover:text-black font-bold transition-colors duration-300">
              <Layers className="w-4 h-4 text-[var(--color-accent)] group-hover:text-black transition-colors" />
              <span>Explore Architecture Case Study</span>
              <ArrowRight className="w-4 h-4 text-[var(--color-accent)] group-hover:text-black group-hover:translate-x-1 transition-all duration-300" />
            </span>
          </Link>
        </motion.div>
      </div>
